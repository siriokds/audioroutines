...
;----------- Get Volume ---
	...
	mov	eax,PAU_MASTERVOL
	imul	ecx
	sar	eax,5
	mov	ecx,eax

	mov	eax,ecx
	mov	ebx,[ebp + KMixChn.Pan]
	imul	eax,ebx
	shl	eax,8
...
;-----------------------------------------------------------------------
	...
	mov	ecx,PAU_BUFFLEN
.C32to16:
	mov	eax,[esi]			; Read L	; 1
	mov	edx,[esi+(PAU_MSAMPLESIZ/2)]	; Read R	; -
	sar	eax,16				; Conv L	; 2
	sar	edx,16				; Conv R	; -
	mov	[edi],ax			; Write L	; 3
	mov	[edi+(PAU_SAMPLESIZ/2)],dx	; Write R	; -
	
	add	esi,PAU_MSAMPLESIZ				; 4
	add	edi,PAU_SAMPLESIZ				; -
	dec	ecx						; 5
	jnz	.C32to16					; 6
